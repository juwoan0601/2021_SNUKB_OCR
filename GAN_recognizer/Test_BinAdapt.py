...
from image_BinAdapt import separateImage, mergeImage
...
for a, size, filename in tqdm(train_dl, desc='GAN Processing Image'):
    with torch.no_grad():
        ...
        for i, img in enumerate(separatedImages):
            numChar = int(image_size[0] / image_size[1] + 0.5)
            img = torch.from_numpy(img)

            img = TF.resize(img, (256, 256))
            img = img.view([1, 3, 256, 256])

            sep_fake_img.append(model_gen(img.to(device)).cpu())

        fake_img = torch.from_numpy(mergeImage(sep_fake_img)).to(device)
        real_img = a

        fake_image = to_pil_image(0.5 * fake_img[0] + 0.5).resize(image_size)
        fakeImg_dir = os.path.join(opt.ImgSave_dir, filename[0])
