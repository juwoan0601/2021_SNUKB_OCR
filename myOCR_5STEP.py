...
def expendBBox(x,y,w,h,shapeOfImage,expendContourRatio):
    #print('Shape', shapeOfImage, 'W', w,'H',h)
    ...
    #print('xe', expended_x,'ye', expended_y,'we',expended_w,'he',expended_h)
...
def OCR_gmobean(RUN_STEPS=[1,2,3,4,5,6], data_dir="./images", result_dir="./result"):
    ...
    startTime = time.time()
    listOfImage = os.listdir(data_dir)
    # STEP 2: GAN image
    if 2 in RUN_STEPS:
        print("STEP2 START {0}s ================".format(int(time.time()-startTime)))
        import os
        GAN_MODEL_NAME = "gen_ep190_batch128_data30060_initHe.pt"
        os.chdir("./GAN_recognizer")
        os.system("python Test_BinAdapt.py --data_dir ../{0} --Img_num {1} --ImgSave_dir ../{2} --model_name {3}".format(data_dir,len(listOfImage),PATH_GAN,GAN_MODEL_NAME))
        os.chdir("../")
    ...
    if 5 in RUN_STEPS:
        ...
        import os
        CR_MODEL_NAME = "./saved_model/best_loss_0.01_model.h5"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--steps', type=int, nargs='+', default=[2,3,4,5,6], help='STEP index to run')
